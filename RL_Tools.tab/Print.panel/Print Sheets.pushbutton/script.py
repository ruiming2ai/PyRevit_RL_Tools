...
class PrintSheetsWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name):
        ...
        self._revision_filter_sheet_ids = set()
    ...
    def _get_revision_sequence(self, revision):
        try:
            return int(getattr(revision, 'SequenceNumber'))
        except Exception:
            try:
                return int(get_elementid_value(revision.Id) or 0)
            except Exception:
                return 0

    def _get_revision_number(self, revision):
        ...
        for bip_name in (
            ...
            try:
                ...
                continue
        return str(self._get_revision_sequence(revision))

    def _get_revision_description(self, revision):
        try:
    ...
    def _collect_filter_revisions(self):
        revisions = \
            DB.FilteredElementCollector(self.selected_doc)\
        ...
        for revision in revisions:
            try:
                rev_id = int(get_elementid_value(revision.Id))
                rev_rows.append(
                    {
                        'id': rev_id,
                        'sequence': self._get_revision_sequence(revision),
                        'number': self._get_revision_number(revision),
                        'description': self._get_revision_description(revision),
                    }
                )
            except Exception:
                continue
        rev_rows.sort(key=lambda x: (x['sequence'], x['number'], x['id']))
        return rev_rows

    def _pick_filter_revisions(self, rev_rows):
        form = WinForms.Form()
        ...
        lv.Width = 880
        lv.Height = 455
        lv.Anchor = (WinForms.AnchorStyles.Top | WinForms.AnchorStyles.Left |
                     WinForms.AnchorStyles.Right | WinForms.AnchorStyles.Bottom)

        lv.Columns.Add("Select", 80)
        lv.Columns.Add("Seq", 70)
        lv.Columns.Add("Number", 160)
        lv.Columns.Add("Description", 540)

        selected_ids = set(self._selected_revision_ids)
        for rev in rev_rows:
            row = WinForms.ListViewItem("")
            row.SubItems.Add(str(rev['sequence']))
            row.SubItems.Add(rev['number'] or "")
            row.SubItems.Add(rev['description'] or "")
            row.Tag = rev
            if rev['id'] in selected_ids:
                row.Checked = True
            lv.Items.Add(row)
        ...
        btn_clr_all.Left = 120
        btn_clr_all.Top = 476
        btn_clr_all.Anchor = WinForms.AnchorStyles.Top | WinForms.AnchorStyles.Left

        save_btn = WinForms.Button()
        save_btn.Text = "Save"
        save_btn.DialogResult = WinForms.DialogResult.OK
        save_btn.AutoSize = True
        save_btn.Left = 775
        save_btn.Top = 476
        save_btn.Anchor = WinForms.AnchorStyles.Top | WinForms.AnchorStyles.Right
        ...
        cancel_btn.Left = 840
        cancel_btn.Top = 476
        cancel_btn.Anchor = WinForms.AnchorStyles.Top | WinForms.AnchorStyles.Right
        ...
        form.Controls.Add(btn_sel_all)
        form.Controls.Add(btn_clr_all)
        ...
        for row in lv.Items:
            try:
                if row.Checked and row.Tag:
                    selected.add(int(row.Tag['id']))
            except Exception:
                continue
        return selected

    def _sheet_has_revision_filter_match(self, sheet_item):
        if not self._selected_revision_ids:
            return False
        try:
            all_rev_ids = set()
            for rev_id in sheet_item.revit_sheet.GetAllRevisionIds():
                rev_int = get_elementid_value(rev_id)
                if rev_int is not None:
                    all_rev_ids.add(int(rev_int))
            return bool(all_rev_ids & self._selected_revision_ids)
        except Exception:
            return False

    def _apply_revision_filter_selection(self):
        self._revision_filter_sheet_ids = set()
        ...
        for sheet_item in self.sheet_list:
            if self._sheet_has_revision_filter_match(sheet_item):
                try:
                    self.sheets_lb.SelectedItems.Add(sheet_item)
                except Exception:
                    pass
                try:
                    self._revision_filter_sheet_ids.add(
                        int(get_elementid_value(sheet_item.revit_sheet.Id))
                    )
                except Exception:
                    continue
    ...
    def options_changed(self, sender, args):
        ...
        self._apply_revision_filter_selection()
    ...
    def filter_by_revisions(self, sender, args):
        del sender, args
        rev_rows = self._collect_filter_revisions()
        if not rev_rows:
            forms.alert('No revisions were found in the selected model.')
            return

        selected_revision_ids = self._pick_filter_revisions(rev_rows)
    ...
    def print_sheets(self, sender, args):
        if self.sheet_list:
            selected_only = False
            if self.combine_print and self._selected_revision_ids:
                selected_only = True
            elif self.selected_sheets:
                opts = forms.alert(
            ...
            if self.combine_print and self._selected_revision_ids \
                    and not target_sheets:
                forms.alert('No sheets match selected revision filters.')
                return
