...
# Revit 2026 removed ElementId.IntegerValue; eid_to_int falls back to Value.
from rltools.compat import eid_to_int as _eid_value
...
class KeynoteManagerWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name, reset_config=False):
        ...
        self._connect_kfile()
        self._preselect_key = self._get_preselect_key_from_selected_tags()
    ...
    @staticmethod
    def _iter_keynote_tree(keynotes):
        for knote in keynotes or []:
            yield knote
            for child in KeynoteManagerWindow._iter_keynote_tree(
                    getattr(knote, 'children', None)):
                yield child

    def _find_keynote_in_tree(self, key):
        for knote in self._iter_keynote_tree(self.current_keynotes):
            if knote.key == key:
                return knote

    def _find_treeview_item(self, parent, target):
        if not parent or not target:
            return

        container = parent.ItemContainerGenerator.ContainerFromItem(target)
        if container:
            return container

        for item in parent.Items:
            parent_container = parent.ItemContainerGenerator.ContainerFromItem(item)
            if parent_container:
                parent_container.IsExpanded = True
                parent_container.UpdateLayout()
                found_item = self._find_treeview_item(parent_container, target)
                if found_item:
                    return found_item

    def _select_keynote_key(self, key):
        target_knote = self._find_keynote_in_tree(key)
        if not target_knote:
            return False

        self.keynotes_tv.UpdateLayout()
        tv_item = self._find_treeview_item(self.keynotes_tv, target_knote)
        if tv_item:
            tv_item.IsSelected = True
            tv_item.BringIntoView()
            return True

        return False

    def _get_preselect_key_from_selected_tags(self):
        try:
            selected_ids = list(revit.uidoc.Selection.GetElementIds())
        except Exception as ex:
            logger.debug('Failed reading selection for keynote preselect | %s', ex)
            return
        ...
        for element_id in selected_ids:
            ...
            key_value = key_param.AsString() if key_param else None
            if key_value and key_value.strip():
                selected_keys.add(key_value.strip())

        if not selected_keys:
            return

        try:
            for knote in kdb.get_keynotes(self._conn):
                if knote.key in selected_keys:
                    return knote.key
        except Exception as ex:
            logger.debug('Failed resolving preselect keynote from selection | %s', ex)

    def _apply_preselection(self):
        if not self._preselect_key:
            return

        try:
            selected_knote = kdb.find(self._conn, self._preselect_key)
        except Exception as ex:
            logger.debug('Failed reading preselect keynote record | %s', ex)
            return

        if not selected_knote:
            return

        if selected_knote.parent_key:
            self._update_ktree(active_catkey=selected_knote.parent_key)
            self._update_ktree_knotes()

        if self._select_keynote_key(self._preselect_key):
            return

        if self.search_term:
            self.search_term = ''
            self._update_ktree_knotes()
            self._select_keynote_key(self._preselect_key)
